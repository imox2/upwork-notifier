...
# to parse the html we get , to extract the data we need
from selenium import webdriver
# will give us a browser which we control by code and go
# to the desired page and get html
import datetime
...
def get_latest_post(time_data, time=60):
    ...
    for time_dict in time_data:
        datetime_obj = datetime.datetime.strptime(time_dict['datetime'],
                                                  "%Y-%m-%dT%H:%M:%S+00:00")
...
if len(time_data) > 0:
    ...
    if number_of_post > 0:
        msg = "Upwork received "+str(number_of_post)+'''
                new queries for scraping'''
        send_msg_to_slack = 1

else:
    msg = '''Upwork notifier seems broken and getting no data.
            Kindly check out the script once'''
    send_msg_to_slack = 1
